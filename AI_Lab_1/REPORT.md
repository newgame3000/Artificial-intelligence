...
Есть некоторое развитвление на разнличные глобальные ситуации (такие как перекресток и контроль движения за навигатором).
...
по схеме факты в одну кучу. (Таким образом получаеются достаточно веселые ситуации :) ). 
...
Также генератор использует классы `Navigator` и `CrossroadPassage` для того, чтобы тут же можно было бы дать ответ на сгенерированные факты.
